...
class KeyManager:
    def __init__(self):
        ...
        self.retention = timedelta(minutes=5)  # Retain keys longer for demo
    ...
    def get_latest_active_key(self) -> Optional[KeyPair]:
        for k in reversed(self.keys):
            if not k.is_expired():
                return k
        return None

    def get_expired_key(self) -> Optional[KeyPair]:
        for k in self.keys:
            if k.is_expired():
                return k
        return None
